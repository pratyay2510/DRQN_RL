...
def plot(csv_file):
    ...
    index_vector = range(len(df))

    plt.figure(figsize=(10,5))

    df["evm_avg"] = df[0].ewm(span=WINDOW_FOR_MOVING_AVG).mean() # calculate the exponentially weighted moving average
    plt.title("Performance Score Variation over Episodes : Frameskip Pong")
    # plt.plot(index_vector, df[0][:num_of_episodes], label="Reward")
    plt.plot(index_vector, df["evm_avg"][:len(df)], label="Reward")
